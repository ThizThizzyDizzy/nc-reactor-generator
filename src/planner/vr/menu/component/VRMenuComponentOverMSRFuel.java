...
public class VRMenuComponentOverMSRFuel extends VRMenuComponent{
    ...
    public VRMenuComponentOverMSRFuel(VRMenuEdit editor, int id, double x, double y, double z, double width, double height, double depth, Fuel fuel, int fuelID){
    ...
    public void renderComponent(TrackedDevicePose.Buffer tdpb){
        ...
        if(editor.getSelectedOverMSRFuel(id).equals(fuel)){
    ...
    public void keyEvent(int device, int button, boolean pressed){
        ...
        if(pressed){
            if(button==VR.EVRButtonId_k_EButton_SteamVR_Trigger){
                editor.selectedOverMSRFuel.put(id, fuelID);
            }
        }
    }
}