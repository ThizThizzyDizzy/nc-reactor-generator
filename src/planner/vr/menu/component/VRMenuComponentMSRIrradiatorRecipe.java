...
public class VRMenuComponentMSRIrradiatorRecipe extends VRMenuComponent{
    ...
    public VRMenuComponentMSRIrradiatorRecipe(VRMenuEdit editor, int id, double x, double y, double z, double width, double height, double depth, IrradiatorRecipe recipe, int recipeID){
    ...
    public void renderComponent(TrackedDevicePose.Buffer tdpb){
        ...
        if(editor.getSelectedMSRIrradiatorRecipe(id).equals(recipe)){
    ...
    public void keyEvent(int device, int button, boolean pressed){
        ...
        if(pressed){
            if(button==VR.EVRButtonId_k_EButton_SteamVR_Trigger){
                editor.selectedMSRIrradiatorRecipe.put(id, recipeID);
            }
        }
    }
}