...
public class VRMenuComponentOverhaulMSRBlockRecipe extends VRMenuComponent{
    ...
    public VRMenuComponentOverhaulMSRBlockRecipe(VRMenuEdit editor, int id, double x, double y, double z, double width, double height, double depth, Block block, BlockRecipe recipe, int recipeID){
    ...
    public void renderComponent(TrackedDevicePose.Buffer tdpb){
        ...
        if(editor.getSelectedOverhaulMSRBlockRecipe(id).equals(recipe)){
    ...
    public void keyEvent(int device, int button, boolean pressed){
        ...
        if(pressed){
            if(button==VR.EVRButtonId_k_EButton_SteamVR_Trigger){
                editor.selectedMSRBlockRecipe.put(id, recipeID);
            }
        }
    }
    @Override    
