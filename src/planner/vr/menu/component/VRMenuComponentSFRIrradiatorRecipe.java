...
public class VRMenuComponentSFRIrradiatorRecipe extends VRMenuComponent{
    private final VRMenuEdit editor;
    private final IrradiatorRecipe recipe;
    private final int id;
    private final int recipeID;
    private float textInset = 0;
    private double textOffset = .001f;//1mm
    public VRMenuComponentSFRIrradiatorRecipe(VRMenuEdit editor, int id, double x, double y, double z, double width, double height, double depth, IrradiatorRecipe recipe, int recipeID){
    ...
    public void renderComponent(TrackedDevicePose.Buffer tdpb){
        ...
        if(editor.getSelectedSFRIrradiatorRecipe(id).equals(recipe)){
    ...
    public void keyEvent(int device, int button, boolean pressed){
        ...
        if(pressed){
            if(button==VR.EVRButtonId_k_EButton_SteamVR_Trigger){
                editor.selectedSFRIrradiatorRecipe.put(id, recipeID);
            }
        }
    }
    @Override    
