...
public class PrimeFuelModule extends Module<Integer>{
    ...
    public Integer calculateMultiblock(Multiblock m){
        if(m instanceof UnderhaulSFR){
            int totalCells = 0;
            ArrayList<multiblock.underhaul.fissionsfr.Block> blocks = m.getBlocks();
            for(multiblock.underhaul.fissionsfr.Block b : blocks){
        ...
        if(m instanceof OverhaulSFR){
            int totalCells = 0;
            ArrayList<multiblock.overhaul.fissionsfr.Block> blocks = m.getBlocks();
            for(multiblock.overhaul.fissionsfr.Block b : blocks){
                if(b.template.fuelCell)totalCells++;
            }
            return totalCells;
        }
        if(m instanceof OverhaulMSR){
            int totalVessels = 0;
            ArrayList<multiblock.overhaul.fissionmsr.Block> blocks = m.getBlocks();
            for(multiblock.overhaul.fissionmsr.Block b : blocks){
