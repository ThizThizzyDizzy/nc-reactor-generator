...
public class FissionSFRConfiguration{
    ...
    public Fuel convertToSFR(planner.configuration.overhaul.fissionmsr.Fuel template){
        if(template==null)return null;
        for(Fuel fuel : fuels){
    ...
    public Source convertToSFR(planner.configuration.overhaul.fissionmsr.Source template){
        if(template==null)return null;
        for(Source source : sources){
            if(source.name.trim().equalsIgnoreCase(template.name.trim()))return source;
        }
