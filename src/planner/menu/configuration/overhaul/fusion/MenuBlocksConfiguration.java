        ...
        for(simplelibrary.opengl.gui.components.MenuComponent c : list.components){
            if(c instanceof MenuComponentBlockConfiguration){
                if(button==((MenuComponentBlockConfiguration) c).delete){
                    configuration.overhaul.fusion.blocks.remove(((MenuComponentBlockConfiguration) c).block);
                    Core.configuration.overhaul.fusion.allBlocks.remove(((MenuComponentBlockConfiguration) c).block);
                    refreshNeeded = true;
                    return;
                }
