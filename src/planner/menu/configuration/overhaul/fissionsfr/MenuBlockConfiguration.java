...
public class MenuBlockConfiguration extends ConfigurationMenu{
    ...
    private final MenuComponentMinimalistButton texture, portInputTexture, portOutputTexture, coolantVentOutputTexture, shieldClosedTexture;
    private final MenuComponentMinimalistTextBox name, displayName, portName, portInputDisplayName, portOutputDisplayName, fuelCellEfficiency, fuelCellHeat, fuelCellCriticality, irradiatorEfficiency, irradiatorHeat, reflectorEfficiency, reflectorReflectivity, moderatorFlux, moderatorEfficiency, shieldHeat, shieldEfficiency, heatsinkCooling, sourceEfficiency, coolantVentOutputDisplayName;
    private final MenuComponentToggleBox blocksLOS, functional, casing, casingEdge, coolantVent, controller, cluster, conductor, createsCluster, fuelCell, fuelCellHasBaseStats, fuelCellSelfPriming, irradiator, irradiatorHasBaseStats, reflector, reflectorHasBaseStats, moderator, moderatorHasBaseStats, moderatorActive, shield, shieldHasBaseStats, heatsink, heatsinkHasBaseStats, source;
    ...
    public MenuBlockConfiguration(GUI gui, Menu parent, Configuration configuration, Block block){
        ...
        functional = add(new MenuComponentToggleBox(sidebar.width, portOutputDisplayName.y+portOutputDisplayName.height, 0, 36, "Functional", false).setTooltip("If set, this block will count against the sparsity penalty"));
        casing = add(new MenuComponentToggleBox(sidebar.width, portOutputDisplayName.y+portOutputDisplayName.height, 0, 36, "Casing", false).setTooltip("If set, this block can be placed in the multiblock casing walls"));
        casingEdge = add(new MenuComponentToggleBox(sidebar.width, portOutputDisplayName.y+portOutputDisplayName.height, 0, 36, "Casing Edge", false).setTooltip("If set, this block can be placed in the multiblock casing edge"));
        controller = add(new MenuComponentToggleBox(sidebar.width, portOutputDisplayName.y+portOutputDisplayName.height, 0, 36, "Controller", false));
        blocksLOS = add(new MenuComponentToggleBox(sidebar.width, functional.y+functional.height, 0, 36, "Blocks Line of Sight", false).setTooltip("If true, this block will count against the sparsity penalty"));
        ...
        fuelCell = add(new MenuComponentToggleBox(sidebar.width, createsCluster.y+createsCluster.height, 0, 48, "Fuel Cell", false, true));
        fuelCellHasBaseStats = add(new MenuComponentToggleBox(fuelCell.x, fuelCell.y+fuelCell.height, 0, 32, "Has base stats", false));
        fuelCellEfficiency = add(new MenuComponentMinimalistTextBox(fuelCell.x, fuelCellHasBaseStats.y+fuelCellHasBaseStats.height, 0, 48, "", true, "Efficiency").setFloatFilter());
        fuelCellHeat = add(new MenuComponentMinimalistTextBox(fuelCell.x, fuelCellEfficiency.y+fuelCellEfficiency.height, 0, 48, "", true, "Heat").setIntFilter());
        fuelCellCriticality = add(new MenuComponentMinimalistTextBox(fuelCell.x, fuelCellHeat.y+fuelCellHeat.height, 0, 48, "", true, "Criticality").setIntFilter());
        fuelCellSelfPriming = add(new MenuComponentToggleBox(fuelCell.x, fuelCellCriticality.y+fuelCellCriticality.height, 0, 32, "Self-Priming", false));
        source = add(new MenuComponentToggleBox(fuelCell.x, fuelCellSelfPriming.y+fuelCellSelfPriming.height, 0, 48, "Neutron Source", false, true));
        ...
        moderator = add(new MenuComponentToggleBox(sidebar.width, createsCluster.y+createsCluster.height, 0, 48, "Moderator", false, true));
        moderatorHasBaseStats = add(new MenuComponentToggleBox(moderator.x, moderator.y+moderator.height, 0, 32, "Has base stats", false));
        ...
        moderatorActive = add(new MenuComponentToggleBox(moderator.x, moderatorFlux.y+moderatorFlux.height, 0, 32, "Active", false).setTooltip("If set, this block will be treated as an active moderator for placement rules"));
        shield = add(new MenuComponentToggleBox(moderator.x, moderatorActive.y+moderatorActive.height, 0, 48, "Neutron Shield", false, true));
        ...
        irradiator = add(new MenuComponentToggleBox(sidebar.width, reflectorReflectivity.y+reflectorReflectivity.height, 0, 48, "Irradiator", false, true));
        irradiatorHasBaseStats = add(new MenuComponentToggleBox(irradiator.x, irradiator.y+irradiator.height, 0, 32, "Has base stats", false));
        ...
        coolantVentOutputTexture = add(new MenuComponentMinimalistButton(shield.x, coolantVent.y+coolantVent.height, 96, 96, "Output Texture", true, true){
            @Override
            public void render(){
                if(block.coolantVentOutputTexture!=null){
                    Core.applyWhite();
                    drawRect(x, y, x+width, y+height, Core.getTexture(block.coolantVentOutputTexture));
                    return;
                }
                super.render();
            }
        ...
        }.setTooltip("Click to change vent output texture\nYou can also drag-and-drop texture files here"));
        ...
        placementRulesLabel = add(new MenuComponentLabel(sidebar.width, Math.max(sourceEfficiency.y+sourceEfficiency.height, Math.max(heatsinkCooling.y+heatsinkCooling.height, coolantVentOutputTexture.y+coolantVentOutputTexture.height)), 0, 48, "Placement Rules", true));
    ...
    public void onGUIOpened(){
        ...
        fuelCellHasBaseStats.isToggledOn = block.fuelCellHasBaseStats;
        fuelCellEfficiency.text = block.fuelCellEfficiency+"";
        fuelCellHeat.text = block.fuelCellHeat+"";
        fuelCellCriticality.text = block.fuelCellCriticality+"";
        fuelCellSelfPriming.isToggledOn = block.fuelCellSelfPriming;
    ...
    public void onGUIClosed(){
        ...
        block.coolantVent = coolantVent.isToggledOn;
        if(block.fuelCell){
            block.fuelCellHasBaseStats = fuelCellHasBaseStats.isToggledOn;
            if(block.fuelCellHasBaseStats){
                block.fuelCellEfficiency = Float.parseFloat(fuelCellEfficiency.text);
                block.fuelCellHeat = Integer.parseInt(fuelCellHeat.text);
                block.fuelCellCriticality = Integer.parseInt(fuelCellCriticality.text);
                block.fuelCellSelfPriming = fuelCellSelfPriming.isToggledOn;
            }else{
                block.fuelCellEfficiency = 0;
                block.fuelCellHeat = 0;
                block.fuelCellCriticality = 0;
                block.fuelCellSelfPriming = false;
            }
        }else{
            block.fuelCellHasBaseStats = false;
            block.fuelCellEfficiency = 0;
            block.fuelCellHeat = 0;
            block.fuelCellCriticality = 0;
            block.fuelCellSelfPriming = false;
        ...
        if(block.coolantVent){
            block.coolantVentOutputDisplayName = coolantVentOutputDisplayName.text.trim().isEmpty()?null:coolantVentOutputDisplayName.text;
        }else{
            block.coolantVentOutputDisplayName = null;
            block.coolantVentOutputTexture = block.coolantVentOutputDisplayTexture = null;
        }
    }
    @Override
    public void tick(){
    ...
    public void render(int millisSinceLastTick){
        ...
        fuelCell.width = fuelCellHasBaseStats.width = fuelCellEfficiency.width = fuelCellHeat.width = fuelCellCriticality.width = fuelCellSelfPriming.width = source.width = sourceEfficiency.width = moderator.width = moderatorHasBaseStats.width = moderatorEfficiency.width = moderatorFlux.width = moderatorActive.width = 
                shield.width = heatsink.width = heatsinkHasBaseStats.width = heatsinkCooling.width = reflector.width = reflectorHasBaseStats.width = reflectorEfficiency.width = reflectorReflectivity.width = 
                irradiator.width = irradiatorHasBaseStats.width = irradiatorEfficiency.width = irradiatorHeat.width = coolantVent.width = (gui.helper.displayWidth()-sidebar.width)/3;
        shieldHasBaseStats.width = shieldEfficiency.width = shieldHeat.width = shield.width-shieldClosedTexture.width;
        coolantVentOutputDisplayName.width = coolantVent.width-coolantVentOutputTexture.width;

        fuelCell.x = heatsink.x = source.x = sidebar.width;//column 1
        reflector.x = moderator.x = coolantVent.x = sidebar.width+(gui.helper.displayWidth()-sidebar.width)/3;//column 2
        irradiator.x = shield.x = sidebar.width+(gui.helper.displayWidth()-sidebar.width)*2/3;//column 3
        
        fuelCellHasBaseStats.x = fuelCellEfficiency.x = fuelCellHeat.x = fuelCellCriticality.x = fuelCellSelfPriming.x = fuelCell.x;
        ...
        coolantVent.x = coolantVentOutputDisplayName.x = coolantVent.x;
        coolantVentOutputTexture.x = coolantVent.x+coolantVent.width-coolantVentOutputTexture.width;
        ...
        fuelCellHasBaseStats.height = fuelCell.isToggledOn?32:0;
        fuelCellEfficiency.height = fuelCellHeat.height = fuelCellCriticality.height = fuelCell.isToggledOn&&fuelCellHasBaseStats.isToggledOn?48:0;
        fuelCellSelfPriming.height = fuelCell.isToggledOn&&fuelCellHasBaseStats.isToggledOn?32:0;
        ...
        coolantVentOutputDisplayName.height = coolantVent.isToggledOn?48:0;
        coolantVentOutputTexture.height = coolantVent.isToggledOn?96:0;
        ...
        fuelCell.y = blocksLOS.y+blocksLOS.height;
        fuelCellHasBaseStats.y = fuelCell.y+fuelCell.height;
        fuelCellEfficiency.y = fuelCellHasBaseStats.y+fuelCellHasBaseStats.height;
        fuelCellHeat.y = fuelCellEfficiency.y+fuelCellEfficiency.height;
        fuelCellCriticality.y = fuelCellHeat.y+fuelCellHeat.height;
        fuelCellSelfPriming.y = fuelCellCriticality.y+fuelCellCriticality.height;
        
        heatsink.y = fuelCellSelfPriming.y+fuelCellSelfPriming.height;
        heatsinkHasBaseStats.y = heatsink.y+heatsink.height;
        heatsinkCooling.y = heatsinkHasBaseStats.y+heatsinkHasBaseStats.height;
        
        source.y = heatsinkCooling.y+heatsinkCooling.height;
        ...
        coolantVent.y = moderatorActive.y+moderatorActive.height;
        coolantVentOutputDisplayName.y = coolantVentOutputTexture.y = coolantVent.y+coolantVent.height;
        ...
        blockRecipesLabel.y = placementRulesLabel.y = Math.max(shieldEfficiency.y+shieldEfficiency.height, Math.max(shieldClosedTexture.y+shieldClosedTexture.height, Math.max(coolantVentOutputDisplayName.y+coolantVentOutputDisplayName.height, Math.max(coolantVentOutputTexture.y+coolantVentOutputTexture.height, sourceEfficiency.y+sourceEfficiency.height))));
