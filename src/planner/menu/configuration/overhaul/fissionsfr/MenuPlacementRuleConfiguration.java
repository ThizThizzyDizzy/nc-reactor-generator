...
public class MenuPlacementRuleConfiguration extends ConfigurationMenu{
    ...
    public void onGUIOpened(){
        ...
        switch(rule.ruleType){
            ...
            case BETWEEN:
            case AXIAL:
                for(Block b : Core.configuration.overhaul.fissionSFR.allBlocks){
                ...
                block.setSelectedIndex(rule.block==null?0:Core.configuration.overhaul.fissionSFR.allBlocks.indexOf(rule.block));
                block.preferredHeight = min.height = max.height = 64;
                addRule.x = placementRules.x = placementRulesLabel.x = -50000;//unless the screen's over 50k wide, this should be good
                break;
            case VERTEX:
    ...
    public void onGUIClosed(){
        ...
        switch(rule.ruleType){
            ...
            case VERTEX_GROUP:
                ...
                break;
            case BETWEEN:
                rule.block = Core.configuration.overhaul.fissionSFR.allBlocks.get(block.getSelectedIndex());
                rule.min = (byte) Math.min(min.getValue(), max.getValue());
                rule.max = (byte) Math.max(min.getValue(), max.getValue());
                break;
            case AXIAL:
