...
public class MenuComponentCoilConfiguration extends MenuComponent{
    public final Coil coil;
    ...
    }.setTooltip("Delete coil\nWARNING: This cannot be undone!"));
    public MenuComponentCoilConfiguration(Coil coil){
        super(0, 0, 0, 100);
        this.coil = coil;
    }
    @Override
    ...
    public void renderForeground(){
        Core.applyColor(Core.theme.getTextColor());
        drawText(x, y, x+width, y+height/4, coil.name);
        if(coil.efficiency>0)drawText(x, y+height/4, x+width, y+height/4*2, "Efficiency: "+coil.efficiency);
        if(coil.connector)drawText(x, y+height/4*2, x+width, y+height/4*3, "Connector");
        if(coil.bearing)drawText(x, y+height/4*3, x+width, y+height/4*4, "Bearing");
    }
}