...
public class MenuPlacementRuleConfiguration extends Menu{
    ...
    public void render(int millisSinceLastTick){
        type.width = coilType.width = coil.width = min.width = max.width = rules.width = back.width = Core.helper.displayWidth();
        type.height = coilType.height = coil.height = min.height = max.height = rules.height = back.height = Core.helper.displayHeight()/16;
        coilType.y = coil.y = min.y = max.y = rules.y = -Core.helper.displayHeight()/8;
