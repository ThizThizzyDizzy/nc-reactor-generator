...
public class PartConfigurationMenu extends ConfigurationMenu{//for blocks, recipes, coolant recipes, etc.
    ...
    protected void addMainSection(String tag, Supplier<Image> getTextureFunc, Consumer<Image> setTextureFunc, String nameTooltip, String displayNameTooltip, Supplier<String> getNameFunc, Supplier<String> getDisplayNameFunc, Supplier<ArrayList<String>> getLegacyNamesFunc, Consumer<String> setNameFunc, Consumer<String> setDisplayNameFunc, Consumer<ArrayList<String>> setLegacyNamesFunc){
        ...
        piles.add(new ComponentPile(){
            ...
            private Label legacyNamesLabel;
            private SingleColumnList legacyNamesList;
            {
                texture = add(new TextureButton(0, 0, 192, 192, tag, true, getTextureFunc, setTextureFunc));
                ...
                legacyNamesLabel = add(new Label(0, 0, 0, 32, "Legacy Names", true).setTooltip("A list of old names for NCPF back-compatibility"));
                legacyNamesList = add(new SingleColumnList(0, 0, 0, 0, 16));
            }
            @Override
            float arrange(float y){
                float w = gui.getWidth()-sidebar.width-texture.width;
            ...
            void onOpened(){
                ...
                for(String s : getLegacyNamesFunc.get()){
                    legacyNamesList.add(new TextBox(0, 0, 0, 48, s, true));
                }
            }
            @Override
            void onClosed(){
                ...
                for(Component c : legacyNamesList.components){
            ...
            void tick(){
                ...
                for(int i = 0; i<legacyNamesList.components.size(); i++){
                    Component comp = legacyNamesList.components.get(i);
                    if(comp instanceof TextBox){
                        if(((TextBox)comp).text.trim().isEmpty()){
                            if(i==legacyNamesList.components.size()-1)hasEmpty = true;
                            else toRemove.add(comp);
                        }
                    }
                }
                if(!hasEmpty)legacyNamesList.add(new TextBox(0, 0, 0, 48, "", true));
