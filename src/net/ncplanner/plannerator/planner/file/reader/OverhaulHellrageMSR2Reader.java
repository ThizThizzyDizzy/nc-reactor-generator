...
public class OverhaulHellrageMSR2Reader implements FormatReader{
    @Override
    ...
    public synchronized NCPFFile read(InputStream in){
        ...
        for(String name : heatSinks.keySet()){
            ...
            for(net.ncplanner.plannerator.multiblock.configuration.overhaul.fissionmsr.Block blok : Core.configuration.overhaul.fissionMSR.allBlocks){
                for(String nam : blok.getLegacyNames())if(StringUtil.superRemove(nam, " ", "coolant", "heater", "liquid").equalsIgnoreCase(StringUtil.superReplace(StringUtil.toLowerCase(name), "water", "standard", " ", "")))block = blok;
            }
