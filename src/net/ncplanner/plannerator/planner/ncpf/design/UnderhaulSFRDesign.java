...
public class UnderhaulSFRDesign extends Design<NCPFUnderhaulSFRDesign> implements MultiblockDesign<UnderhaulSFR>{
    ...
    public UnderhaulSFRDesign(NCPFFile file, int x, int y, int z){
        this(file);
        design = new Block[x+2][y+2][z+2];
    }
    @Override
    public void convertFromObject(NCPFObject ncpf){
        super.convertFromObject(ncpf);
        fuel = copy(definition.fuel, Fuel::new);
        copy3DArray(definition.design, design = new Block[definition.design.length][definition.design[0].length][definition.design[0][0].length], Block::new);
    }
    @Override
        ...
        for(int x = 0; x<design.length; x++){
            for(int y = 0; y<design[x].length; y++){
                for(int z = 0; z<design[x][y].length; z++){
                    ...
                    sfr.setBlock(x, y, z, new net.ncplanner.plannerator.multiblock.underhaul.fissionsfr.Block(file.conglomeration, x, y, z, design[x][y][z]));
                }
            }
        }
        return sfr;
