...
public class FissionMSRConfiguration extends AbstractBlockContainer<Block> {
    ...
    public void apply(FissionMSRConfiguration partial, ArrayList<Multiblock> multiblocks, PartialConfiguration parent){
        ...
        for(Multiblock mb : multiblocks){
            if(mb instanceof OverhaulMSR){
                for(net.ncplanner.plannerator.multiblock.overhaul.fissionmsr.Block b : ((OverhaulMSR)mb).getBlocks(true)){
    ...
    public Block convertToMSR(net.ncplanner.plannerator.multiblock.configuration.overhaul.fissionsfr.Block template) throws MissingConfigurationEntryException{
        if(template==null)return null;
        for(Block block : allBlocks){
            if(block.name.equals(StringUtil.superReplace(template.name, "solid", "salt", "cell", "vessel", "sink", "heater")))return block;
        }
        for(Block block : blocks){
            if(block.name.equals(StringUtil.superReplace(template.name, "solid", "salt", "cell", "vessel", "sink", "heater")))return block;
        }
    ...
    public void convertAddon(AddonConfiguration parent, Configuration convertTo) throws MissingConfigurationEntryException{
        if(convertTo.overhaul.turbine==null)throw new IllegalArgumentException("Cannot convert to null Overhaul MSR configuration!");
        for(Block block : blocks){
