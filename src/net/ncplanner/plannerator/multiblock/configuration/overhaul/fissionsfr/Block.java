...
public class Block extends RuleContainer<PlacementRule.BlockType, Block> implements Pinnable, IBlockTemplate {
    ...
    public Config save(Configuration parent, FissionSFRConfiguration configuration, boolean partial){
        ...
        if(isHereJustToHoldRecipes){
            ...
            if(coolantVent){
                ...
                if(!partial)LegacyNCPFWriter.saveTexture(coolantVentCfg, "outTexture", coolantVentOutputTexture);
    ...
    public BlockRecipe convertToSFR(net.ncplanner.plannerator.multiblock.configuration.overhaul.fissionmsr.BlockRecipe template){
        if(template==null)return null;
