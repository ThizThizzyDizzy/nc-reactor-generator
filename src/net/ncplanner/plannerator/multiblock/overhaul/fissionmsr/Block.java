...
public class Block extends AbstractBlock{
    ...
    public net.ncplanner.plannerator.multiblock.overhaul.fissionsfr.Block convertToSFR() throws MissingConfigurationEntryException{
        ...
        net.ncplanner.plannerator.multiblock.overhaul.fissionsfr.Block b = new net.ncplanner.plannerator.multiblock.overhaul.fissionsfr.Block(getConfiguration(), pos, newTemplate);
        if(irradiatorRecipe!=null){
    ...
    public Block addNeutronSource(OverhaulMSR msr, BlockElement source){
        ...
        for(Direction d : Direction.values()){
            ...
            while(true){
                i++;
                if(!msr.contains(pos.offset(d,i))){
                    possible.put(pos.offset(d,i-1), i);
                    break;
                }
                Block b = msr.getBlock(pos.offset(d,i));
        ...
        for(BlockPos key : keys){
            Block was = msr.getBlock(key);
            if(tryAddNeutronSource(msr, source, key))return was==null?msr.getBlock(key):was;
        }
        return null;
    }
    private boolean tryAddNeutronSource(OverhaulMSR msr, BlockElement source, BlockPos pos){
        Block b = msr.getBlock(pos);
        if(b!=null&&(b.isController()||b.template.port!=null||b.template.neutronSource!=null))return false;
        msr.setBlock(pos, new Block(msr.getConfiguration(), pos, source));
        return true;
    }
    @Override
