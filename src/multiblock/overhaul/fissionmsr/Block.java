...
public class Block extends multiblock.Block{
    ...
    public String getTooltip(Multiblock multiblock){
        ...
        if(isShield()){
            tip+="\nShield "+(shieldActive?"Valid":"Invalid")+"\n"
                    ...
                    + "Total heat: "+(flux*template.heatMult)+" H/t\n"
                    + "Efficiency factor: "+template.efficiency;
        }
        if(closed){
            tip+="\nClosed";
        ...
        for(Direction d : directions){
            ...
            for(int i = 1; i<=reactor.getConfiguration().overhaul.fissionMSR.neutronReach+1; i++){
                ...
                if(block.isReflector()){
                    ...
                    if(flux>0)vesselGroup.positionalEfficiency+=efficiency/length*block.template.efficiency;
                ...
                if(block.isIrradiator()){
                    ...
                    if(block.irradiatorRecipe!=null){
                        if(flux>0)vesselGroup.positionalEfficiency+=efficiency/length*block.irradiatorRecipe.efficiency;
                    }
                    break;
                }
                break;
        ...
        for(Direction d : directions){
            ...
            for(int i = 1; i<=reactor.getConfiguration().overhaul.fissionMSR.neutronReach+1; i++){
                ...
                if(block.isReflector()){
                    ...
                    for(Block b : shieldFluxes.keySet()){
                        b.flux+=flux*(1+block.template.reflectivity);
                    }
                    for(Block b : toActivate)b.moderatorActive = true;
                    for(Block b : toValidate)b.moderatorValid = true;
                    break;
    ...
    public void convertTo(Configuration to){
        if(template.fuelVessel)fuel = to.overhaul.fissionMSR.convert(fuel);
        if(template.fuelVessel)source = to.overhaul.fissionMSR.convert(source);
        if(template.irradiator)irradiatorRecipe = to.overhaul.fissionMSR.convert(irradiatorRecipe);
